...
export default class StarterSelectUiHandler extends MessageUiHandler {
    ...
    if (button === Button.SUBMIT) {
      ...
      if (button === Button.ACTION) {
        if (!this.speciesStarterDexEntry?.caughtAttr) error = true;
          ...
          const options = [
            {
              label: i18next.t("starterSelectUiHandler:ADD_TO_PARTY"),
              handler: () => {
            ...
            {
              label: i18next.t("starterSelectUiHandler:TOGGLE_IVS"),
              handler: () => {
          ...
          if (this.speciesStarterMoves.length > 1) {
                ...
                ui.showText("Select a move to swap out.", null, () => {
                  ui.setModeWithoutClear(Mode.OPTION_SELECT, {
                    options: moveset
                      .map((m: Moves, i: number) => {
                        const option: OptionSelectItem = {
                          ...
                          handler: () => {
                            ui.setMode(Mode.STARTER_SELECT).then(() => {
                              ui.showText(
                                `Select a move to swap with ${allMoves[m].name}.`,
                                null,
                                () => {
                                  ui.setModeWithoutClear(Mode.OPTION_SELECT, {
                      ...
                      .concat({
                        label: i18next.t("starterSelectUiHandler:Cancel"),
                        handler: () => {
                          this.clearText();
                          ui.setMode(Mode.STARTER_SELECT);
                          return true;
            ...
            options.push({
              label: i18next.t("starterSelectUiHandler:MANAGE_MOVES"),
              handler: () => {
                showSwapOptions(this.starterMoveset);
                return true;
              },
          ...
          if (passiveAttr & PassiveAttr.UNLOCKED) {
            if (!(passiveAttr & PassiveAttr.ENABLED)) {
              options.push({
                label: i18next.t("starterSelectUiHandler:ENABLE_PASSIVE"),
                handler: () => {
              ...
              options.push({
                label: i18next.t("starterSelectUiHandler:DISABLE_PASSIVE"),
                handler: () => {
                  starterData.passiveAttr ^= PassiveAttr.ENABLED;
                  ui.setMode(Mode.STARTER_SELECT);
                  this.setSpeciesDetails(
            ...
            if (!(passiveAttr & PassiveAttr.UNLOCKED)) {
              ...
              options.push({
                label: `x${passiveCost} ${i18next.t(
                  "starterSelectUiHandler:UNLOCK_PASSIVE"
                )} (${
                  allAbilities[
            ...
            if (valueReduction < 2) {
              ...
              options.push({
                label: `x${reductionCost} ${i18next.t(
                  "starterSelectUiHandler:REDUCE_COST"
                )}`,
                handler: () => {
          ...
          if (!pokemonPrevolutions.hasOwnProperty(this.lastSpecies.speciesId)) {
            options.push({
              label: i18next.t("starterSelectUiHandler:USE_CANDIES"),
              handler: () => {
  ...
  updateInstructions(): void {
    ...
    if (this.speciesStarterDexEntry?.caughtAttr) {
      if (this.canCycleShiny) cycleInstructionLines.push("R: Cycle Shiny");
      if (this.canCycleForm) cycleInstructionLines.push("F: Cycle Form");
      if (this.canCycleGender) cycleInstructionLines.push("G: Cycle Gender");
      if (this.canCycleAbility) cycleInstructionLines.push("E: Cycle Ability");
      if (this.canCycleNature) cycleInstructionLines.push("N: Cycle Nature");
      if (this.canCycleVariant) cycleInstructionLines.push("V: Cycle Variant");
    }
    ...
    for (let cil of cycleInstructionLines) instructionLines.push(cil);
  ...
  setSpeciesDetails(
    ...
    if (species) {
      this.dexAttrCursor |= (
        shiny !== undefined ? !shiny : !(shiny = oldProps.shiny)
      )
        ? DexAttr.NON_SHINY
        : DexAttr.SHINY;
      this.dexAttrCursor |= (
        female !== undefined ? !female : !(female = oldProps.female)
      )
        ? DexAttr.MALE
        : DexAttr.FEMALE;
      this.dexAttrCursor |= (
        variant !== undefined ? !variant : !(variant = oldProps.variant)
      )
        ? DexAttr.DEFAULT_VARIANT
        : variant === 1
        ? DexAttr.VARIANT_2
      ...
      if (
        ...
        this.canCycleForm =
          species.forms
            ...
            .map(
              (_, f) => dexEntry.caughtAttr & this.scene.gameData.getFormAttr(f)
            )
            .filter((f) => f).length > 1;
        this.canCycleNature =
            ...
            originalStarterSelectCallback(
              new Array(this.starterGens.length).fill(0).map(function (_, i) {
                ...
                return {
                  ...
                  passive: !(
                    thisObj.scene.gameData.starterData[starterSpecies.speciesId]
                      .passiveAttr ^
